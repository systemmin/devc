...
func main() {
	top := flag.Bool("top", false, "[窗口] 置顶当前 CMD 窗口")
	han := flag.Int("han", 4, "[关键字翻译] 汉语关键字长度，默认支持4个汉字，最少2个汉字。例如：订单编号、订单状态、创建时间")
	hump := flag.Int("hump", 1, "[关键字翻译] 将翻译内容写入剪切板，命名规则：1.大驼峰 2.小驼峰 3.下划线")
	tran := flag.Bool("tran", false, "[翻译工具] 开启自动检测翻译，关键字翻译将关闭")
	w := flag.Bool("w", false, "[翻译工具|关键字翻译] 将翻译内容写入剪切板")
	api := flag.String("api", "http://127.0.0.1:11434", "ollama api")
	...
	for data := range watch {
		...
		if *tran {
			translate := internal.Translate(*api, *model, data)
			fmt.Println("原文->", data)
			...
			if textType == internal.CHINESE { // 中文
				...
				if len(line) > 0 {
					...
					translate := internal.Translate(*api, *model, word)
